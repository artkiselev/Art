...
Telegram-бот для сбора свадебной анкеты пары. Бот задает вопросы по очереди, сохраняет прогресс, формирует Word-файл и отдельный промпт для подготовки текста церемонии через Codex.
