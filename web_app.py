...
def questionnaire_page(session: WebSession) -> HTMLResponse:
    ...
    question = QUESTIONS[session.current_index]
    progress = int((session.current_index / question_count()) * 100)
    eyebrow = "Изменение ответа" if session.status == "editing" else f"Вопрос {session.current_index + 1} из {question_count()}"
    body = f"""
    <section class="shell">
      <header class="topbar">
        <div>
          <div class="brand">Dream Wedding</div>
          <div class="muted">Личная анкета для церемонии</div>
        </div>
        <form method="post" action="/reset"><button class="ghost" type="submit">Новая анкета</button></form>
      </header>
      <main class="workspace">
        <aside class="rail">
          <div class="metric"><span>{session.current_index + 1}</span><small>текущий вопрос</small></div>
          <div class="metric"><span>{question_count()}</span><small>всего вопросов</small></div>
          <div class="progress"><i style="width:{progress}%"></i></div>
        </aside>
        <section class="panel">
          <div class="eyebrow">{escape(eyebrow)}</div>
          <h1>{escape(question.prompt)}</h1>
          <p class="section-label">{escape(question.section)}</p>
          <form class="answer-form" method="post" action="/answer">
            ...
            <div class="actions">
              <button class="primary" type="submit">{'Сохранить изменение' if session.status == 'editing' else 'Сохранить ответ'}</button>
              <button class="secondary" id="recordButton" type="button">Голосовой ответ</button>
            </div>
...
def review_page(session: WebSession) -> HTMLResponse:
        ...
        answers.append(
            ...
            <article class="answer-card">
              ...
              <form method="post" action="/edit">
                ...
                <button class="icon-button" title="Изменить ответ" aria-label="Изменить ответ {index}" type="submit">✎</button>
              </form>
            </article>
            """
        )
    ...
    <section class="shell">
      <header class="topbar">
        <div>
          <div class="brand">Dream Wedding</div>
          <div class="muted">Проверьте финальную анкету</div>
        </div>
        <form method="post" action="/reset"><button class="ghost" type="submit">Новая анкета</button></form>
      </header>
      <main class="review-layout">
        <section class="review-head">
          ...
          <h1>Если все верно, подтвердите анкету. Если нужно исправить ответ, нажмите значок рядом с вопросом.</h1>
          <form method="post" action="/confirm">
            <button class="primary wide" type="submit">Подтвердить и передать ведущему</button>
          </form>
        </section>
...
def login_view(message: str = "") -> str:
    ...
    <section class="login-screen">
      <main class="login-panel">
        <div class="brand">Dream Wedding</div>
        <h1>Личная анкета для свадебной церемонии</h1>
        <p>Введите код доступа, который вам передал ведущий.</p>
        {alert}
        <form method="post" action="/login">
          <input name="code" autocomplete="one-time-code" placeholder="Код доступа" required>
...
def success_view() -> str:
    return """
    <section class="login-screen">
      <main class="login-panel success-panel">
        <div class="brand">Dream Wedding</div>
        ...
        <form method="post" action="/reset">
          <button class="secondary wide" type="submit">Заполнить новую анкету</button>
        </form>
      </main>
    </section>
    """
...
def failure_view() -> str:
    ...
    <section class="login-screen">
      <main class="login-panel">
        ...
        <p>Попробуйте подтвердить анкету чуть позже или напишите ведущему.</p>
        <a class="secondary link-button" href="/review">Вернуться к анкете</a>
      </main>
    </section>
    """
